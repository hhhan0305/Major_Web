...
class Cat {
  ...
  Draw() {
    ...
    if (keys["KeyA"]) {
      img.src = "cat_L.png";
      ctx.drawImage(img, (this.x -= 3), this.y, this.w, this.h);
    } else if (keys["KeyD"]) {
      img.src = "cat_R.png";
      ctx.drawImage(img, (this.x += 3), this.y, this.w, this.h);
    } else if (keys["KeyW"]) {
      img.src = "cat.png";
      ctx.drawImage(img, this.x, (this.y -= 3), this.w, this.h);
    } else {
      img.src = "cat.png";
      ctx.drawImage(img, this.x, this.y, this.w, this.h);
    }
    // var img = new Image();
    // img.src = "image/cat_up.png";
    // ctx.drawImage(img, this.x, this.y, this.w, whis.h);
  }
...
function SpawnObstacle() {
  ...
  if (type == 1) {
    newObstacle.y -= cat.originalHeight - 10;
...
function Start() {
  ...
  cat = new Cat(canvas.width / 2, canvas.height - 100, 100, 100, "black");
  //cat.Draw();// <- 업데이트 함수에서 그리기 위해서 삭제
...
function Update() {
  ...
  cat.Animate(); //공룡한테 애니메이션 주는 함수 <-여기서 그려줄거임
